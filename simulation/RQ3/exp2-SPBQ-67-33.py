...
new_actor_prob = [actor_prob[i] for i in reverse_ids]
new_business_prob = [business_prob[i] for i in reverse_ids]
new_path_prob = [path_prob[i] for i in reverse_ids]
new_actor_prob.extend([actor_prob[i] for i in range(len(actor_prob)) if i not in reverse_ids])
new_business_prob.extend([business_prob[i] for i in range(len(business_prob)) if i not in reverse_ids])
new_path_prob.extend([path_prob[i] for i in range(len(path_prob)) if i not in reverse_ids])
...
for i in range(0, 1):
    ...
    file_name = path + "/tmp_result/" + "67-33-SPBQ-s-"
    # print(file_name)
    ...
    file_name = path + "/tmp_result/" + "67-33-SPBQ-sc-"
    # print(file_name)
    AEU, ASR, TPS, ACU, last_time, AEU_dict = run(simulate_dict, exp_path, people_arr, actor_prob, business_prob, path_prob, actor_business, file_name)
    ...
    file_name = path + "/tmp_result/" + "67-33-SPBQ-a-"
    # print(file_name)
    AEU, ASR, TPS, ACU, last_time, AEU_dict = run(simulate_dict, exp_path, people_arr, actor_prob, business_prob, path_prob, actor_business, file_name)
    ...
    file_name = path + "/tmp_result/" + "67-33-SPBQ-ac-"
    # print(file_name)
    AEU, ASR, TPS, ACU, last_time, AEU_dict = run(simulate_dict, exp_path, people_arr, actor_prob, business_prob, path_prob, actor_business, file_name)
...
with open(path + "/result/exp2-SPBQ-67-33.txt", "w") as f:
